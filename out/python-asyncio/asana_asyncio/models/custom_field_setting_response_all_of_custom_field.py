...
class CustomFieldSettingResponseAllOfCustomField(BaseModel):
    """
    ...
    is_value_read_only: Optional[StrictBool] = Field(default=None, description="*Conditional*. This flag describes whether a custom field is read only.")
    created_by: Optional[UserCompact] = None
    people_value: Optional[List[UserCompact]] = Field(default=None, description="*Conditional*. Only relevant for custom fields of type `people`. This array of [compact user](/reference/users) objects reflects the values of a `people` custom field.")
    privacy_setting: Optional[StrictStr] = Field(default=None, description="The privacy setting of the custom field. *Note: Administrators in your organization may restrict the values of `privacy_setting`.*")
    default_access_level: Optional[StrictStr] = Field(default=None, description="The default access level when inviting new members to the custom field. This isn't applied when the `privacy_setting` is `private`, or the user is a guest. For local fields in a project or portfolio, the user must additionally have permission to modify the container itself.")
    ...
    __properties: ClassVar[List[str]] = ["gid", "resource_type", "name", "type", "enum_options", "enabled", "representation_type", "id_prefix", "is_formula_field", "date_value", "enum_value", "multi_enum_values", "number_value", "text_value", "display_value", "description", "precision", "format", "currency_code", "custom_label", "custom_label_position", "is_global_to_workspace", "has_notifications_enabled", "asana_created_field", "is_value_read_only", "created_by", "people_value", "privacy_setting", "default_access_level", "resource_subtype"]
    ...
    def format_validate_enum(cls, value):
        ...
        if value not in set(['currency', 'identifier', 'percentage', 'custom', 'duration', 'none']):
    ...
    def privacy_setting_validate_enum(cls, value):
        ...
        if value not in set(['public_with_guests', 'public', 'private']):
            raise ValueError("must be one of enum values ('public_with_guests', 'public', 'private')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in people_value (list)
        _items = []
        if self.people_value:
            for _item_people_value in self.people_value:
                if _item_people_value:
                    _items.append(_item_people_value.to_dict())
            _dict['people_value'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "is_value_read_only": obj.get("is_value_read_only"),
            "created_by": UserCompact.from_dict(obj["created_by"]) if obj.get("created_by") is not None else None,
            "people_value": [UserCompact.from_dict(_item) for _item in obj["people_value"]] if obj.get("people_value") is not None else None,
