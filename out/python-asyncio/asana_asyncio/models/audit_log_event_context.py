...
class AuditLogEventContext(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["context_type", "api_authentication_method", "client_ip_address", "user_agent", "oauth_app_name"]
    ...
    def context_type_validate_enum(cls, value):
        ...
        if value not in ('web', 'desktop', 'mobile', 'asana_support', 'asana', 'email', 'api'):
    ...
    def api_authentication_method_validate_enum(cls, value):
        ...
        if value not in ('cookie', 'oauth', 'personal_access_token', 'service_account'):
