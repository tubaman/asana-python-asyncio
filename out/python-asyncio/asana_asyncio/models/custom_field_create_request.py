...
class CustomFieldCreateRequest(BaseModel):
    ...
    workspace: StrictStr = Field(description="*Create-Only* The workspace to create a custom field in.")
    owned_by_app: Optional[StrictBool] = Field(default=None, description="*Allow-listed*. Instructs the API that this Custom Field is app-owned. This parameter is allow-listed to specific apps at this point in time. For apps that are not allow-listed, providing this parameter will result in a `403 Forbidden`.")
    people_value: Optional[List[StrictStr]] = Field(default=None, description="*Conditional*. Only relevant for custom fields of type `people`. This array of user GIDs reflects the users to be written to a `people` custom field. Note that *write* operations will replace existing users (if any) in the custom field with the users specified in this array.")
    resource_subtype: StrictStr = Field(description="The type of the custom field. Must be one of the given values.")
    __properties: ClassVar[List[str]] = ["gid", "resource_type", "name", "type", "enum_options", "enabled", "representation_type", "id_prefix", "is_formula_field", "date_value", "enum_value", "multi_enum_values", "number_value", "text_value", "display_value", "description", "precision", "format", "currency_code", "custom_label", "custom_label_position", "is_global_to_workspace", "has_notifications_enabled", "asana_created_field", "workspace", "owned_by_app", "people_value", "resource_subtype"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "owned_by_app": obj.get("owned_by_app"),
            "people_value": obj.get("people_value"),
